...
Not all events in a .fcs file are created equally. Events collected during periods of turbulent flow may have erroneously high or low fluorescence values. These periods may also be sparse with data. Removing events from untrustworthy periods of acquisition is important in any analysis to ensure they don't result in false conclusions. The flowClean, flowAI algorithms are two methods for cleaning up flow cytometry files, that are available in flowJo. A newer algorithm, *flowCut* is available in R that seems to remove regions of data that I would consider removing manually myself, so we'll use this instead (with their default parameters I find flowClean can be a little lax, while flowAI can be a little strict). An alternative algorithm in R is PeacoQC, which is newer, but I don't find it performs as well as flowCut on my data.

To apply the flowCut algorithm to a flowFrame, it's as simple as loading the package and calling `flowCut([flowFrame])`. We can extract the cleaned flowframe from the resulting object using `$frame`.
...
cut_result$worstChan #for just the worst channel, useful to visualise any problems by using a plotting tool
```

As we wish to continue working with the cleaned flowFrame, we can extract the frame slot, and store it as a new object.

```{r clean_fcs}
clean_fcs <- cut_result$frame
```
...
```{r compensate}
comp_fcs <- compensate(clean_fcs, spillover(clean_fcs)[[1]])
```
...
The code below passes each flowFrame to the flowCut algorithm and returns the cleaned flowFrame. The resulting clean_fs object is, conveniently, a new flowSet containing the cleaned flowFrames.

> REMEMBER: We can define a function we're not going to use again on the fly. This is called an *anonymous function*.

```{r}
clean_fs <- fsApply(fs, function(x) {
...
```{r comp_fs}
comp_matrix <- spillover(clean_fs[[1]])[[1]]

comp_fs <- compensate(clean_fs, comp_matrix)
```
