...
Finally, to help us assign cell types to our clusters ("annotating" the clusters), let's plot the UMAP embeddings, but facetting by antigen, and colouring by expression of that antigen.
...
The process of assigning a biological cell type to each cluster is usually the hardest and labour intensive part of the analysis. To help with this, in addition to the UMAP plots we created a moment ago, it can be useful to create an expression heatmap for all the clusters. This is a heatmap where rows represent FlowSOM metaclusters, columns represent antigens, and the colour of each tile represents the median expression value.
