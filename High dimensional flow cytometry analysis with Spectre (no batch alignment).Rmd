...
cell_dat <- run.flowsom(cell_dat, transformed_cols, meta.k = 15)
```
...
cell_sub <- run.umap(cell_sub, transformed_cols)
```
...
```{r metak_heatmap}
medians <- do.aggregate(cell_dat, transformed_cols, by = "FlowSOM_metacluster")

make.pheatmap(medians, 
              sample.col = "FlowSOM_metacluster", 
              plot.cols = transformed_cols,
              path = "plots")
```
...
annots <- list("T cells"             = c(1, 4),
               ...
               "Mature B cells"      = 2,
               "Monocytes"           = c(13, 14),
               "Mature neutrophils"  = c(15, 12),
               "Immature neutrophils"= 9,
               "Other/Unknown"       = c(5, 7, 11)
)
...
names(cell_dat)[11:18] <- simple_names
```
